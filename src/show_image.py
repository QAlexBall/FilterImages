...
from utils.fetch_images_info import set_image, get_all_image, _update_current_image_id
...
class ShowImage(QWidget):
    ...
    def __init__(self):
        ...
        self.load_path = QPushButton("LoadPath", self)
        self.reload_path = QPushButton("ReloadPath", self)
    ...
    def setUpLayout(self):
        ...
        if self.path != "default":
            ...
            self.image_info_label.setText("current_image_id: {}".format(current_image_id))
            self.image_info_layout.addWidget(self.image_info_label)
        else:
            pass
        ...
        # add widget
        self.button_layout.addWidget(self.load_path)
        self.button_layout.addWidget(self.reload_path)
    ...
    def addButtonClick(self):
        ...
        self.load_path.clicked.connect(self.load_remote_path)
        self.reload_path.clicked.connect(self.reload_remote_path)
    ...
    def load_remote_path(self):
        ...
        print(data_info)
        if ok and path_exist and data_info is None:
            self.path = path
            get_all_image(self.ssh_client, self.collection, folder_name=path)
            _update_current_image_id(self.collection, 'init')
        else:
            self.path = path
            self.collection = self.db[path]
        print("load finished.")
